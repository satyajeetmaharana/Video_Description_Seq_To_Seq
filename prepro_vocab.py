...
def main(params):
    ...
    all_video_ids = os.listdir('vatex/val')
    video_ids = {}
    for video_feat_file in all_video_ids:
        video_feat_np = np.load(os.path.join('vatex/val', video_feat_file))
        if (video_feat_np.shape[1] == 32):
            video_ids[video_feat_file.split('.')[0]] = True
    for i in videos:
        #Get video feats. Keep only those with 32 size
