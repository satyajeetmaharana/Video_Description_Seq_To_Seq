...
class VideoDataset(Dataset):
    ...
    def __init__(self, opt, mode):
        ...
        self.video_ids = []
        for video_id in self.splits['train']:
            self.video_ids.append('video' + str(video_id) + '.npy')
        for video_id in self.splits['val']:
            self.video_ids.append('video' + str(video_id) + '.npy')
        for video_id in self.splits['val']:
            self.video_ids.append('video' + str(video_id) + '.npy')
    ...
    def __getitem__(self, ix):
        ...
        fc_feat = []
        video_feat_file = self.video_ids[ix]
        for dir in self.feats_dir:
            fc_feat.append(np.load(os.path.join(dir, video_feat_file)))
            #print(np.load(os.path.join(dir, video_feat_file)).shape)
        fc_feat = np.concatenate(fc_feat, axis=1)
        if self.with_c3d == 1:
            c3d_feat = np.load(os.path.join(self.c3d_feats_dir, video_feat_file))
        ...
        captions = self.captions[video_feat_file.split('.')[0]]['final_captions']
        ...
        data['video_ids'] = video_feat_file.split('.')[0]
        return data
